  ...
  return (
    <div className="max-w-7xl mx-auto pb-12">
      ...
      <motion.div
        ...
        className="bg-gray-900/80 backdrop-blur-sm border border-cyan-500/30 rounded-2xl p-8 mb-8 shadow-lg shadow-cyan-500/20"
      >
          ...
          <div className="space-y-3">
            ...
            <div className="relative">
              <select
                ...
                className="w-full bg-gray-800/80 border border-cyan-500/40 rounded-xl px-4 py-3 text-cyan-100 
                         focus:border-cyan-400 focus:outline-none appearance-none cursor-pointer
                         hover:border-cyan-300 transition-colors"
              >
                {daysOfWeek.map(day => (
                  <option key={day} value={day} className="bg-gray-800 text-cyan-100">
                    {day}
                  </option>
                ))}
              </select>
          ...
          {showStats && (
            ...
            >
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-gray-800/60 border border-cyan-500/30 rounded-xl p-4 text-center">
                  <div className="text-2xl font-bold text-cyan-400">{stats.totalCombos}</div>
                  <div className="text-sm text-cyan-300">Total Combos</div>
                </div>
                <div className="bg-gray-800/60 border border-cyan-500/30 rounded-xl p-4 text-center">
                  <div className="text-2xl font-bold text-cyan-400">{stats.avgCalories}</div>
                  <div className="text-sm text-cyan-300">Avg Calories</div>
                </div>
                <div className="bg-gray-800/60 border border-cyan-500/30 rounded-xl p-4 text-center">
                  <div className="text-2xl font-bold text-cyan-400">{stats.avgPopularity}%</div>
                  <div className="text-sm text-cyan-300">Avg Popularity</div>
                </div>
                <div className="bg-gray-800/60 border border-cyan-500/30 rounded-xl p-4 text-center">
                  <div className="text-2xl font-bold text-cyan-400 capitalize">{stats.favoriteTaste}</div>
                  <div className="text-sm text-cyan-300">Favorite Taste</div>
                </div>
              </div>
            </motion.div>
          )}
      ...
      >
        <p className="text-cyan-100 text-lg">
          <span className="text-cyan-400 font-semibold">{selectedDay}</span> • Session {currentSession}
        </p>
      ...
      <AnimatePresence>
        {error && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -20 }}
            className="bg-red-900/20 border border-red-500/40 rounded-xl p-4 mb-6 text-center"
          >
